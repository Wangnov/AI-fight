...
LOGO_LOCK="Use the logo reference images as architectural emblems, stained-glass symbols, holographic wall inlays, or large back-wall signage motifs: OpenAI knot for Altman/cyan lab, Anthropic AI mark and Claude star for Dario/safety cathedral, Grok circle-lightning mark for Elon/aerospace launch control. The emblems must feel physically built into the arena, not placed inside floating cards, UI panels, stickers, posters, or detached rectangles."
...
Create a new opening/menu background for AI Fight with a true all-star fighting-game attract-screen feeling. Use the reference for the rain-soaked stadium mood and preserve the original iconic brand-emblem feeling, but broaden it to include three competing worlds in one dramatic arena: cyan frontier AI lab architecture with an OpenAI emblem built into the left structure, amber gothic safety tribunal architecture with Anthropic/Claude emblems built into the central stained glass, and violet-blue aerospace launch control architecture with a Grok emblem built into the right hangar. Leave the lower foreground clear for three full-body fighter poses added by the game. The composition should feel like a crossover roster stage, not a poster. ${LOGO_LOCK} ${LAYOUT_LOCK}"

ALTMAN_DARIO_PROMPT="${STYLE_LOCK}

Regenerate the Altman vs Dario arena. Preserve the original stage identity: left side has a large OpenAI knot emblem integrated into cyan frontier AI lab architecture; right side has a large Anthropic AI / Claude safety emblem integrated into amber-purple gothic tribunal stained glass. Center seam should be a clean duel line, with a glossy floor reflecting both worlds. Do not draw floating logo cards or rectangular UI plaques. ${LOGO_LOCK} ${LAYOUT_LOCK}"

ALTMAN_ELON_PROMPT="${STYLE_LOCK}

Generate an Altman vs Elon arena using the existing stage as style reference. Left side: cyan AI research lab and model-training control room with a large OpenAI knot emblem built into a glass terminal wall. Right side: violet-blue aerospace launch hangar with rocket gantries, electric telemetry screens, and a large Grok circle-lightning emblem built into the command-wall architecture. Make it feel like lab access versus launch velocity. Do not draw floating logo cards or rectangular UI plaques. ${LOGO_LOCK} ${LAYOUT_LOCK}"

DARIO_ELON_PROMPT="${STYLE_LOCK}

Generate a Dario vs Elon arena using the existing stage as style reference. Left side: amber/purple safety cathedral courtroom with Anthropic AI / Claude emblem stained glass, formal audit architecture. Right side: violet-blue launch command bunker with rocket gantry silhouettes, hard industrial lights, and a large Grok circle-lightning emblem built into machinery or glass. Make the center feel like a tense hearing inside a launch facility. Do not draw floating logo cards or rectangular UI plaques. ${LOGO_LOCK} ${LAYOUT_LOCK}"

MIRROR_PROMPT="${STYLE_LOCK}

Generate a neutral mirror-match arena for any same-character fight. It should be a symmetrical AI coliseum at night: split-color neon lighting, cyan on the left and orange-violet on the right, central storm-lit fighting platform, abstract circuitry and gothic arches blended into one balanced stage. Include subtle integrated emblem reliefs in the architecture, but no floating cards, panels, or plaques. ${LOGO_LOCK} ${LAYOUT_LOCK}"
